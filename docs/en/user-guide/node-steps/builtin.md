...
The URL can contain [Context Variables](/en/user-guide/job-workflows.md#context-variables) that will be expanded at runtime.
...
Job References can be run as either _Node Steps_ or _Workflow Steps_ (see [Workflow Steps : Types of Steps](/en/user-guide/job-workflows.html#workflow-steps)).
...
Source Path
: Path on the QW Control server for the file or base directory (recursive/wildcard search).
...
For [Script steps](/en/#script-step), [Script file steps](/en/#script-file-step), and [Script URL steps](/en/#script-url-step), you can specify an optional _Invocation_ string to declare how the script should be executed.
...
This will then allow your script to make use of [Sudo authentication](/en/administration/projects/node-execution/ssh.md#secondary-sudo-password-authentication).
...
When you define a [Command](/en/#command-step) or arguments to any Script or Job reference step, your arguments are interpreted as a space-separated sequence of strings. If you need to use spaces or quotes within the argument, here are some rules for quoting arguments:
