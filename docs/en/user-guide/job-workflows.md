...
[Reordering steps](/en/#reordering-steps)
...
- Node Steps operate once on each Node, which could be multiple times within a workflow. For a full list of Node Steps, see [Job Plugins - Node Steps](/en/user-guide/job-plugins.md#node-steps)
- Workflow Steps operate only once in the workflow. For a full list of Workflow Steps, see [Workflow Steps](/en/user-guide/job-plugins.md#workflow-steps)
...
See the section on [Context Variables](/en/#context-variables) for more information.
...
Option context variables are referred to as `option.NAME` (more about [Job Options](/en/user-guide/job-options.md).)

There may be additional context variables available, such as `data.*` values when using the [Data Capture Job Filter Plugins](/en/#data-capture-job-filter-plugins).
...
::: tip Note
See the chapter [Configuring Remote Machine for SSH](/en/administration/projects/node-execution/ssh.md#configuring-remote-machine-for-ssh) for information about requirements of the SSH server.
:::
...
- [Exporting Jobs](/en/api/qwcontrol-api.md#exporting-jobs)
- [Importing Jobs](/en/api/qwcontrol-api.md#importing-jobs)
- [RD CLI Tool][rd]
