...
(See [Jobs - Log Limit](/en/user-guide/04-jobs.md#log-limit)). Allowed values:
...
: "node-first" or "step-first". Determines the strategy for executing the sequence across a set of nodes. See the [QW Control User Manual](/en/user-guide/04-jobs.md#workflow-control-settings) for more info.

`commands`

: This is a Sequence of: \* One or more [Command Definitions](/en/#command)

### Command

Each command in the [Sequence](/en/#sequence) can be of these different types:

- [Simple command execution entry](/en/#simple-command-entry)
- [Script execution entry](/en/#script-execution-entry)
- [Script file execution entry](/en/#script-file-execution-entry)
- [Job Reference entry](/en/#job-reference-entry)
- [Plugin step entry](/en/#plugin-step-entry)

Each command can also embed an [Error Handler](/en/#error-handler).

Each command can have a [description](/en/#description).
...
The Error Handler contents can be exactly the same as a [Command](/en/#command), except it
...
This [Command](/en/#command) consists of a single entry:
...
This [Command](/en/#command) executes the script content specified.
...
This [Command](/en/#command) executes a script file stored on the server.
...
This [Command](/en/#command) downloads a script file from a URL and executes it.
...
This [Command](/en/#command) executes another QW Control Job.
...
: map consisting of these entries:
    ...
    [nodefilters](/en/#job-reference-nodefilters)
...
The `nodefilters` should contain a `filter` entry. The value is a string defining a node filter. See [User Guide - Node Filters](/en/user-guide/11-node-filters.md).
...
This [Command](/en/#command) executes a plugin. There are two types of step plugins: Node step, and Workflow step.
...
Options for a job can be specified with a list of Maps. Each map contains a `name` key with the name of the option, and the content is a map defining the [Option](/en/#option).
...
If the [Options](/en/#options) are defined in a list, the order specified will be preserved.
...
- See the [Jobs Guide](/en/user-guide/04-jobs.md#remote-option-values) for more info.
