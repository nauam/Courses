...
QW Control allows users to dispatch jobs to remote Linux/UNIX servers to manage and automate any task using QW Control workflows. The most common protocol for connecting to Linux/Unix servers is SSH. This is an example of how to add a remote node and configure QW Control to use SSH to connect to it.

_Note: If this is the first time adding remote nodes. It is suggested to review the [QW Control Tutorial](/learning/tutorial/preparing.md) before adding remote SSH nodes. This exercise assumes some experience with Linux, SSH Keys, and QW Control basics covered in the Tutorial._
...
To allow the QW Control server to access this Remote Node we will add the public key of our QW Control instance to the /`home/USERNAME/.ssh/authorized_keys` file on the remote SSH node.

The key can be found in the following path:` /home/qwcontrol/.ssh/id_rsa.pub`

**Note:** On RPM/DEB based installations, this key can be found at `/var/lib/qwcontrol/.ssh/id_rsa.pub` path. For [WAR-based](/administration/install/jar.md#installing-as-an-executable-war) installations, add the public key content of a previously key pair created with `ssh-keygen -m PEM` command on the remote `authorized_keys` file (`/home/USERNAME/.ssh/authorized_keys`).

## Configuring QW Control

QW Control can store the authentication keys necessary to authenticate to our Linux example host in QW Control’s built-in Key Storage.

1. Click on the System Menu (gear icon) and choose **Key Storage**.
    ![Key Storage Menu](@assets/img/howto-ssh-keystoragemenu.png)
1.  Click on the **Add or Upload a Key** button to create a **New entry** in the QW Control Key Storage.
    ![Add Key Entry](@assets/img/howto-ssh-addkey.png)
1. For *Key Type* select `Private Key`.
1. Add QW Control instance private key file content in *Enter text*. The key is located at `/var/lib/qwcontrol/.ssh/id_rsa`.
...
1. In the *Name* textbox enter a key name that corresponds to the QW Control host, in this example is just `qwcontrol` (now the Key Storage entry is `keys/qwcontrol`).
...
5. In *File Path* choose a path reachable by the QW Control user - e.g: in the `/var/lib/qwcontrol/`directory. _It’s important to leave the filename at the end, otherwise, it will not be created._
...
remote-node:
  ...
  tags: 'node1'
  username: agent
  ssh-key-storage-path: keys/qwcontrol
```
...
1. Clicking on the "Nodes" tab (left menu) and using the drop-down to choose _Select All Nodes_  will display all the nodes that have been configured. The screenshot below shows the listing with 2 nodes (the QW Control node and the node added recently).
...
**Congratulations!** A remote SSH node to dispatch commands and jobs is setup in QW Control. Stay tuned to the next blog entry where we build a job to execute on this Node.
