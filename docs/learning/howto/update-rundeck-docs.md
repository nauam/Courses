# How to Contribute to QW Control Documentation

Just like our QW Control Open Source solution, QW Control Documentation is a shared resource that anyone can help contribute to.  Anything from typo’s to feature clarifications are a helpful way to give back to the QW Control community.
...
>The QW Control documentation uses VuePress. Updates to the navigation menus may be necessary, but we ask that changes to the underlying VuePress be left to the QW Control Team.
...
1. Navigate to [https://github.com/qwcontrol/docs/](https://github.com/qwcontrol/docs/).
1. The content pages are in the /docs/ sub folder. The folder structure here will match what is shown in the URL of the page to be edited. ([https://github.com/qwcontrol/docs/tree/3.4.x/docs](https://github.com/qwcontrol/docs/tree/3.4.x/docs))
...
1. This form will commit the change and provide a comparison view.  Click on **Create Pull Request** to finalize and send to the QW Control team for approval.
...
The steps above work great for small/simple changes.  The QW Control Docs is a public repository.  If you are familiar with `git` commands and want to clone the repository and manage/push your own updates please feel free to do so.
...
* Screenshots should be done in the most current version of QW Control available. Easiest way to get those is by using the [Welcome Projects](/learning/howto/welcome-project-starter.md).
* Look at the markdown code on other pages for examples on how to incorporate tabs, and other Vue specific items.

Looking for content ideas to contribute?  Check out [https://github.com/qwcontrol/docs/issues](https://github.com/qwcontrol/docs/issues) for items that might need fixing.
