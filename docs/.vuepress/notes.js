...
const { QW ControlVersion } = require('./version.js')
...
async function main() {
    const context = {}
    context.core = await getRepoData({ repo: 'qwcontrol', owner: 'qwcontrol' }, [])
    context.enterprise = await getRepoData({ repo: 'qwcontrolpro', owner: 'qwcontrolpro' }, ['release-notes/include'])
    context.docs = await getRepoData({ repo: 'docs', owner: 'qwcontrol' }, [])
    context.systemreport = await getRepoData({ repo: 'enterprise-system-report', owner: 'qwcontrolpro' }, ['release-notes/include'])
    context.schedules = await getRepoData({ repo: 'enterprise-schedules', owner: 'qwcontrolpro' }, ['release-notes/include'])
    context.calendars = await getRepoData({ repo: 'enterprise-calendars', owner: 'qwcontrolpro' }, ['release-notes/include'])
    context.tours = await getRepoData({ repo: 'enterprise-tour-manager', owner: 'qwcontrolpro' }, ['release-notes/include'])
    ...
    context.version = new QW ControlVersion({ versionString: argv.milestone })
